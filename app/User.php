...
class User extends Authenticatable implements TwoFactorAuthenticatableContract, JWTSubject
{
    ...
    public function gravatar()
    {
        ...
        if(!empty($this->attributes['avatar'])){
            if(file_exists(public_path().'/upload/users/'.$this->attributes['avatar']))
                return asset('upload/users/'.$this->attributes['avatar']);
            else
    ...
    {
        return $this->hasMany(NewsUpdates::class,'user_id','id');
    }

    public function tickets(){
        return $this->belongsToMany(SupportTicket::class,'support_user');
    }
}
