...
class DeliveryScheduleController extends Controller
{
    ...
    public function add(Request $request){
        if($request->has('location') && $request->has('stock_no')){
            ...
            ksort($locations);
            //dd($locations);
            if($request->location==5 || $request->location==1){
                $locationarray = [5,1];
            }else{
                $locationarray = [$request->location];
            }

            $salesPersons = Userdetails::where('fld_usr_cat',3)->whereIn('fld_usr_location',$locationarray)->get()->pluck('fullname','id')->toArray();
            ...
            //$salesPersons = array_push($salesPerson, $samtoy);
            ...
            $turnOvers = Userdetails::where('fld_usr_cat',16)->orderBy('fld_usr_lastname','asc')->get()->pluck('full_name','id')->toArray();
    ...
    public function FfDelivery(Request $request){
        if($request->has('location') && $request->has('stock_no')){
            $this->validate($request,[
                'location'=>'required|exists:locations,id',
                //'stock_no'=>'required|exists:vehicles,fldStockNo'
            ...
            //dd($trackingTypes);
            return view('delivery-schedule.add.step-ff',compact('vehicle','trackingTypes','locations','salesPersons','turnOvers','payments'));
        }else{
            $locations = Locations::all();
            return view('delivery-schedule.add.step-1',compact('locations'));
        }
    ...
    public function addFfDelivery(DeliveryRequest $request){
        //Add to the table
        //dd(($request->all((new Deliveryff())->getFillable())));
        $delivery = Deliveryff::create($request->all((new Deliveryff())->getFillable()));
        ...
        return redirect(route('ff-delivery'))->withSuccess(trans('success.add-delivery'));
    }
    ...
    public function edit(Delivery $delivery){
        ...
        if($delivery->fld_location==5 || $delivery->fld_location==1){
            $locationarray = [5,1];
        }else{
            $locationarray = [$delivery->fld_location];
        }
        $salesPersons = Userdetails::where('fld_usr_cat',3)->whereIn('fld_usr_location',$locationarray)->get()->pluck('fullname','id')->toArray();
        $salesPersons[''] = 'Choose One';
        ksort($salesPersons);
    ...
    public function service(Request $request, Locations $location)
    {
        ...
        $service_techs = Userdetails::where('fld_usr_location', $locationID)->where('fld_usr_cat', 12)->get()->pluck('tech_name', 'tech_name')->toArray();
