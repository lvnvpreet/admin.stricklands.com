...
class BarcodeController extends Controller
{
    ...
    /**
     * Displays List oF All Barcode base on location code.
     *
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function listByLocation(Request $request){
        switch ($request->input('location')) {
            case 'E':   $locationName = 'BC-Auto Mart';             break;
            case 'T':   $locationName = 'BC-Toyota';                break;
            case 'NH':  $locationName = 'BC-Not Here';              break;
            case 'B':   $locationName = 'BC-Budget Lot';            break;
            case 'IT':  $locationName = 'BC-In Transit';            break;
            case 'DL':  $locationName = 'BC-Demo Loaner';           break;
            case 'BG':  $locationName = 'BC-GMC';                   break;
            case 'W':   $locationName = 'BC-Windsor';               break;            
            default:    $locationName = 'Stricklands New Arrivals'; break;
        }

        $vehiclesQuery = Vehicles::where('fldSoldStatus','0')
                                ->where('fldKey1','');
        if($request->has('location')){
            $vehiclesQuery->where('fldLocationCode',$request->input('location'));
        }

        $vehicles = $vehiclesQuery->orderBy('fldStockNo','asc')
                                ->orderBy('fldYear','desc')
                                ->orderBy('fldMake','desc')
                                ->orderBy('fldModel','desc')
                                ->orderBy('fldModelNo','desc')
                                ->get();

        return view('dashboard.list-barcode',compact('locationName','vehicles'));
    }
    ...
    public function renderBarcode(Request $request){
        ...
        // Startup code
        //-----------------------------------------------------------------------------
        if($request->has('text'))       $text   =   $request->input('text');
        if($request->has('format'))     $format =   $request->input('format');
        if($request->has('quality'))    $quality=   $request->input('quality');
        if($request->has('width'))      $width  =   $request->input('width');
        if($request->has('height'))     $height =   $request->input('height');
        if($request->has('type'))       $type   =   $request->input('type');
        if($request->has('barcode'))    $barcode=   $request->input('barcode');
