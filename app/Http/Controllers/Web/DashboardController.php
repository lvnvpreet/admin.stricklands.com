...
use Vanguard\Models\Deliveryff;
...
class DashboardController extends Controller
{
    ...
    private function adminDashboard()
    {
        ...
        $statsFFQuery = Deliveryff::where('fld_date','>=',$locations->get(100)->day_start)
            ->where('s_spare','No')->get();
        $stats = [];
        ...
        $stats['stricklands_automart']['title_ff'] = 'New';
        ...
        $stats['stricklands_automart']['target'] = $locations->get(5)->fldStoreTarget ?? 0+$locations->get(1)->fldStoreTarget ?? 0;
        $stats['stricklands_automart']['percent'] = round(($stats['stricklands_automart']['sale']/$stats['stricklands_automart']['target']) * 100);
    
        $stats['stricklands_automart']['ffsale'] =  $statsFFQuery->wherein('fld_location',[5,1])->where('fld_funded','Yes')->where('fld_new_used',1)->count();
        $stats['stricklands_automart']['fftarget'] = $locations->get(5)->fldStoreNewTarget ?? 0;
        $stats['stricklands_automart']['ffpercent'] = round(($stats['stricklands_automart']['fftarget'] == 0) ? 0 : ($stats['stricklands_automart']['ffsale']/$stats['stricklands_automart']['fftarget']) * 100);


        $stats['stricklands_windsor']['title'] = 'Strickland\'s Windsor';
        $stats['stricklands_windsor']['title_ff'] = 'New';
        $stats['stricklands_windsor']['id'] = '2';
        $stats['stricklands_windsor']['sale'] = $statsQuery->where('fld_location',2)->where('fld_funded','Yes')->whereIn('fld_new_used',['2','3'])->count();
        $stats['stricklands_windsor']['target'] = $locations->get(2)->fldStoreTarget ?? 0;
        $stats['stricklands_windsor']['percent'] = round(($stats['stricklands_windsor']['sale']/$stats['stricklands_windsor']['target']) * 100);
    
        $stats['stricklands_windsor']['ffsale'] =  $statsFFQuery->where('fld_location',2)->where('fld_funded','Yes')->where('fld_new_used',1)->count();
        $stats['stricklands_windsor']['fftarget'] = $locations->get(2)->fldStoreNewTarget ?? 0;
        $stats['stricklands_windsor']['ffpercent'] = round(($stats['stricklands_windsor']['fftarget'] == 0) ? 0 : ($stats['stricklands_windsor']['ffsale']/$stats['stricklands_windsor']['fftarget']) * 100);

        $stats['stricklands_toyota_new']['title'] = 'Stratford Toyota';
        $stats['stricklands_toyota_new']['title_ff'] = 'New';
        ...
        $stats['stricklands_toyota_new']['target'] =$locations->get(5)->fldStoreTarget ?? 0 +$locations->get(1)->fldStoreTarget ?? 0;
        $stats['stricklands_toyota_new']['percent'] = round(($stats['stricklands_toyota_new']['sale']/$stats['stricklands_toyota_new']['target']) * 100);
    
        $stats['stricklands_toyota_new']['ffsale'] = $statsQuery->where('fld_location',5)->where('fld_funded','Yes')->where('fld_new_used',1)->count();
        $stats['stricklands_toyota_new']['fftarget'] = $locations->get(5)->fldStoreNewTarget ?? 0;
        $stats['stricklands_toyota_new']['ffpercent'] = round(($stats['stricklands_toyota_new']['fftarget'] == 0) ? 0 : ($stats['stricklands_toyota_new']['ffsale']/$stats['stricklands_toyota_new']['fftarget']) * 100);
        ...
        $stats['brantford_gmc']['title_ff'] = 'New';
        ...
        $stats['brantford_gmc']['sale'] = $statsQuery->where('fld_location',8)->where('fld_funded','Yes')->where('fld_new_used','<>',1)->where('fld_tracker_type','<>',13)->count();

        $stats['brantford_gmc']['target'] = $locations->get(8)->fldStoreTarget ?? 0;
        $stats['brantford_gmc']['percent'] = round(($stats['brantford_gmc']['sale']/$stats['brantford_gmc']['target']) * 100);
    
        $stats['brantford_gmc']['ffsale'] =  $statsQuery->where('fld_location',8)->where('fld_funded','Yes')->where('fld_new_used',1)->where('fld_tracker_type','<>',13)->count();
        $stats['brantford_gmc']['fftarget'] = $locations->get(8)->fldStoreNewTarget ?? 0;
        $stats['brantford_gmc']['ffpercent'] = round(($stats['brantford_gmc']['fftarget']) ? 0 : ($stats['brantford_gmc']['ffsale']/$stats['brantford_gmc']['fftarget']) * 100);
    ...
    public function SavevehicleDescription($stock, Request $request){

        $vehicle = Vehicles::where('fldStockNo',$stock)->first();
        if(is_null($vehicle)) abort(404);

        if($request->filled('engine_desc')){
            if($vehicle->vehicle_data){
                $vehicle->vehicle_data()->update(['engine_desc'=>$request->get('engine_desc')]);
            }else{
                $vehicle->vehicle_data()->create(['engine_desc'=>$request->get('engine_desc')]);
            }
