...
class UsersController extends Controller
{
    ...
    public function create(CountryRepository $countryRepository, RoleRepository $roleRepository)
    {
        ...
        $oldlevels =   \DB::table('old_usr_level')->pluck('usr_lev','usr_lev_id')->toArray();
        $teamRoles = getAllTeams();
        $user = \Auth::user();
        return view('user.add', compact('user','countries', 'roles', 'statuses','positions','locations','oldlevels','teamRoles'));
    }
    ...
    public function store(CreateUserRequest $request)
    {
      ...
      //  dd($request);
        ...
        $user->details()->create([
            ...
            'fld_usr_level'=>$request->get('fld_usr_level'),
            'fld_logistics_level'=>$request->get('fld_logistics_level'),
            'fld_tradein_level'=>$request->get('fld_tradein_level'),
            'fld_auction_level'=>$request->get('fld_auction_level'),
            'fld_hr_level'=>$request->get('fld_hr_level'),
            'fld_team_role'=>$request->get('fld_team_role'),
        ]);
    ...
    public function edit(User $user, CountryRepository $countryRepository, RoleRepository $roleRepository)
    {
        ...
        return view(
            'user.edit',
            compact('edit', 'user', 'countries', 'socialLogins', 'roles', 'statuses','positions','locations','oldlevels','teamRoles')
        );
    }
    ...
    public function updateDetails(User $user, UpdateDetailsRequest $request)
    {
        ...
        if(Auth::user()->role_id == 1 || Auth::user()->role_id == 2 || Auth::user()->role_id == 8){
            ...
            $data['fld_team_role']=$request->get('fld_team_role');
            $data['fld_is_team'] = $request->get('fld_is_team');
