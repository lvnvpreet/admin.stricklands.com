            ...
            <div id="vehicles-wrap" class="panel panel-default panel-table">
                <div class="panel-body">
                    <table class="table table-hover table-bordered zero-configuration">
                        ...
                        <th width="6%" class="head1">W.O.</th>
                        <th width="6%" class="head1">P Tech</th>
                        <th width="6%" class="head1">S Tech</th>
                        <th width="6%" class="head1">E-Test</th>
                        <th width="6%" class="head1">Safety</th>
                        <th width="6%" class="head1">Re-Ceck</th>
                        ...
                        @if(isset($deliveries) && count($deliveries))
                            @foreach($deliveries as $delivery)
                                <tr style="background-color: {{ $delivery->tbl_bg_color }}">
                                    ...
                                    <td>{!! Form::text('s_w_o_'.$delivery->id,$delivery->s_w_o,["class"=>"form-control","onchange"=>"update_delivery(this,'s_w_o',".$delivery->id.")"]) !!}</td>
                                    <td>{!! Form::select('s_p_tech_'.$delivery->id,$service_techs,$delivery->s_p_tech,["class"=>"form-control","onchange"=>"update_delivery(this,'s_p_tech',".$delivery->id.")"]) !!}</td>
                                    <td>{!! Form::select('s_tech_'.$delivery->id,$service_techs,$delivery->s_tech,["class"=>"form-control","onchange"=>"update_delivery(this,'s_tech',".$delivery->id.")"]) !!}</td>
                                    <td class="{{ ($delivery->s_emmision == 'Yes') ? 'alert-success' : 'alert-danger' }}" >{!! Form::select('s_emmision_'.$delivery->id,['No'=>'No','Yes'=>'Yes'],$delivery->s_emmision,["class"=>"form-control","onchange"=>"update_delivery(this,'s_emmision',".$delivery->id.")"]) !!}</td>
                                    <td class="{{ ($delivery->s_safe == 'Yes') ? 'alert-success' : 'alert-danger' }}">{!! Form::select('s_safe_'.$delivery->id,['No'=>'No','Yes'=>'Yes'],$delivery->s_safe,["class"=>"form-control","onchange"=>"update_delivery(this,'s_safe',".$delivery->id.")"]) !!}</td>
                                    <td>{!! Form::select('s_recheck_'.$delivery->id,['SS'=>'SS','RC'=>'RC'],$delivery->s_recheck,["class"=>"form-control","onchange"=>"update_delivery(this,'s_recheck',".$delivery->id.")"]) !!}</td>
